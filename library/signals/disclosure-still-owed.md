...
kind: risk
confidence: graded
inputs: [compliance-fence, conversation-history, deal]
counts: [required-disclosures, disclosures-on-record, decision-language-used]
answers: [V18]
needs: []
assumes: [thread-under-way]
assumes_status: provisional
pull: Organisation's compliance fence, for what must always be said before a buyer agrees
---
...
- Confidence: high when the rule is a hard one in the fence and the thread carries the buyer's own
  ...
  passing cue.
- Needs: no source beyond the fence and the thread. The fence itself is a told thing: if the
  ...
  A disclosure made on an uncaptured call is invisible until you say so.
