---
id: cast
about: the people, real to the fiction
---

# The cast

Everyone a card can be about. `you` is the seller the tool is speaking to.

Two halves, and they are the other way round from the patient sale. There, one seller faced several
people at each buyer. Here there is **one owner per business** and **two people on your own side**:
you until the offers are in, your processor after that. Nobody is sure where the line falls, which
is the scenario's own open question and not a gap in the library.

| Person | id | Role | Organisation | What you know |
|---|---|---|---|---|
| You | you | the rep | — | You work a deal from the first yes until the offers are in. Fast, plain, no small talk. You text more than you email |
| Marcus Bell | marcus-bell | processor at your firm | — | Takes a deal on once the documents are complete: picks the funders, submits, collects the offers and presents them. Will not submit an incomplete file |
| Dee Whitlock | dee-whitlock | owner | sandhill-diner | Funded in February, repaying daily since. Answers texts within the hour |
| Tony Prieto | tony-prieto | owner | corner-tire | Three offers in since Friday. You have not passed them on yet |
| Alma Vega | alma-vega | owner | vega-landscaping | Submitted this morning. Takings drop hard from November to February and she said so herself |
| Renee Sackville | renee-sackville | owner | bluebird-childcare | Signed the application on Thursday. Sent two months of statements, not three |
| Errol Danning | errol-danning | owner | mercer-dental | Said yes on a text on Friday afternoon. Nothing since |
| Paula Larkin | paula-larkin | owner | larkin-print | Accepted Kestrel's offer on Friday. Waiting on the contract |
| Wes Halloran | wes-halloran | owner | halsey-freight | Funded in June. Two debits returned unpaid this month |
| Joyce Amadi | joyce-amadi | owner | oak-row-bakery | Funded in January, most of the way through. Took an advance from someone else last time before you called |
| Curtis Naylor | curtis-naylor | owner | pinewood-cleaners | Declined by three funders in May on his credit score. Back on the form last week |
| Ivy Sandoval | ivy-sandoval | owner | fenwick-auto-body | Funded on Friday, the money on her account the same day. Nothing owed and nothing asked for yet |
| Hannah Beck | hannah-beck | funding manager | avondale-capital | Your usual first call on a restaurant. Answers submissions same day |
