...
// you; the list had nothing written for a customer, because no card to one had met it), or the
// step their side has shown (added 22 September, from the firm's world: a note that offers help
// with a buyer's own process opens on the step they have visibly taken, which is theirs and on
// record, never on the steps they have not, which the read says it cannot see).
const openers = new Set(['auto-reply-no-longer-with', 'colleague-touches-on-record', 'documents-received', 'how-far-through-the-term', 'agreed-payments-not-on-record', 'steps-with-a-sign', 'promise-made-undelivered', 'their-question-unanswered', 'things-they-asked-you-for',
...
for (const c of L.cards) {
  ...
  for (const r of rows) {
    ...
    if (r[0] === 'opens with' && !rests.some(id => openers.has(id) || (!c.arrives && reach.has(id))))
      problems.push(`${c.file}: the message opens on ${rests.join(', ') || 'nothing'}. It has to open on what is owed, a fresh reason, their own date, the angle, a worry in their own words, what they sent, the step their side has shown, or, after the sale, their own clock or what their account did (${[...openers].join(', ')}), or on what came back when a watch turned the card up`);
