...
> **Act on Tom Reynolds at Council 14 today.** He's quietly become your strongest
...
> the thread. Lead with the records-import answer and the Borsetshire case study, and
...
- *who to act on* — Tom, surfaced by Relationships
...
Brain weighed Tom against the rest of the board, and his situation won on a few counts:
...
*"Tom is a warming champion with a live question"* is really **three smaller readings
stacked together**, and each one rests on specific raw facts.

**"Tom is a champion"** — read from a behaviour pattern, every line a plain fact
...
it:
- **Tom (service-design lead) — champion.** Read from the behaviour above.
- **Priya (head of IT) — newly involved, stance unknown.** Read from her first
...
- the tempo: tough, multi-year, low urgency — which is itself an instruction to nurture
...
pile.** You don't tell the tool that Tom is a champion. The tool reads that off his
...
but aimless. Put both piles together and you get a card that says: act on Tom, today,
...
This assembly is now a live view in the blueprint app — the **Assembly** tab
(`/assembly`), the canvas's complement to the route trace. It builds the picture from
the bottom up: the raw facts appear first, then the grey-to-blue "reading" line, then
the readings, the insight, the gathered pieces, and finally the action glowing at the
apex. (The words there are mirrored from this doc — keep the two in sync.)
