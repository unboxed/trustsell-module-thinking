...
- **Sensing** (the voice read, on the three floors of [`tracing-back.md`](../docs/tracing-back.md):
  ...
  fact that sharpens the read. The dials learn the same way, told first, inferred ever after.

When the Brain hands down a brief (who to act on, why, the points to make), Profile is the final
render step: it pitches the closeness and register to the relationship by *pulling* the history
from People, stays inside the fence by *pulling* the hard limits from Organisation, and writes the
points in your voice. The brief carries the *what and why*; Profile supplies the *how it sounds*.
