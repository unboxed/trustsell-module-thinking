...
window.LIBRARY = {
 ...
 "world": {
  ...
  "goal": {
   ...
   "sections": {
    "Today": {
     "html": "<p>Monday 14 September 2026. Every <code>when</code> on a card is relative to this date.</p>"
    },
    "The ladder": {
 ...
 "questions": [
  ...
  {
   "id": "Q2",
   "text": "**The triage mechanism.** Q21 (\"who most urgently?\") confirms triage is central — but *how*"
  },
  {
   "id": "Q3",
 ...
 "modules": [
  ...
  },
  {
   ...
   "intro": "<p>The personal layer: your voice and your preferences, so every message sounds like you.</p>\n<p>&gt; Profile's catalog floors live as libraries: raw data in &gt; <a href=\"../channels/\"><code>01-integrations/channels/</code></a>, what you tell it in &gt; <a href=\"told.md\"><code>told.md</code></a>, assemblies in <a href=\"assemblies/\"><code>assemblies/</code></a>, signals in &gt; <a href=\"signals/\"><code>signals/</code></a>. This file holds only the operating prose. See &gt; <a href=\"../docs/library-format.md\"><code>docs/library-format.md</code></a>.</p>",
   "sections": {
    "Principles": {
 ...
 "told": [
  ...
  },
  {
   ...
   "connected": true,
   "icon": "pencil",
   "title": "What you tell Organisation",
 ...
 "assemblies": [
  ...
  },
  {
   ...
   "sections": {
    ...
    "Lineage": {
     "html": "<p><code>pitch</code> resolves to <a href=\"../told/offering-told.md\"><code>told.md</code></a>, the offering's own told source. <code>file-content</code> resolves to <a href=\"../channels/drive.md\"><code>channels/drive.md</code></a>. <code>web-result</code> resolves to <a href=\"../channels/web.md\"><code>channels/web.md</code></a>. <code>offering-entry</code> is the sibling assembly this stands on. A claim that lives only in a deck not uploaded to Drive is a <strong>data gap</strong>, named not faked: the claim cannot be grounded until the file is connected.</p>"
    }
   },
   "sectionOrder": [
    "What it gathers",
 ...
 "signals": [
  ...
  },
  {
   ...
   "sections": {
    ...
    "For the build": {
     "html": "<p>Applies the shared reading rules in <a href=\"../docs/reading-principles.md\"><code>docs/reading-principles.md</code></a>.</p>\n<ul><li>Trigger: a bounce-back or a \"no longer with\" auto-reply on the thread is near-binary and fires directly; otherwise silence must dwarf this person's own established cadence (own-rhythm, never a fixed number of days) AND the innocent-explanation scan must come back empty. Only ever for a contact the was-once-active check confirms.</li><li>Confidence: high, because a bounce and a departure auto-reply are facts on record, parsed not inferred; the silence-without-a-bounce branch is held to the guarded own-rhythm test and graded down so it never reads as certain as a bounce.</li><li>Needs: Calendar and Meet, so a future meeting or a recent call counts as a live channel and stops the read landing on someone you just spoke to; without a delivery failure or a departure note, a silent leaver cannot be told apart from a contact who is simply ignoring you, so the read thins to \"went quiet, possibly left\" (a data gap, named not faked).</li></ul>"
    }
   },
   "sectionOrder": [
    "What it means",
  ...
  },
  {
   ...
   "sections": {
    "What it means": {
     "html": "<p>Given a lead's situation, how well it matches this offering's fit-shape, and exactly where it falls short.</p>\n<p>This is a Plant read, made per candidate. Offerings owns the shape and the match. People owns sweeping the address book and calls this read on each lead. It is the mirror of People pulling Offerings to name an objection: here Offerings pulls People to know the lead.</p>\n<p>A miss is named, not scored away. A lead that meets every factor but the one you marked essential reads weaker than one that misses two minor ones.</p>"
    },
    "In practice": {
  ...
  },
  {
   ...
   "sections": {
    "What it means": {
     "html": "<p>A fresh face arrives in a conversation you are already in, and the map gains a name it did not have.</p>\n<p>The read fires on a name that has no history: on no earlier email, on no earlier event, and not already on the organisation's map. That empty history is what tells a real arrival apart from a colleague who has just been quiet for a while.</p>\n<p>Seniority matters here. A \"Head of\" signature or an organiser role on an invite weighs more than a name buried in a long Cc.</p>"
    },
    "In practice": {
  ...
  },
  {
   ...
   "sections": {
    ...
    "For the build": {
     "html": "<p>Applies the shared reading rules in <a href=\"../docs/reading-principles.md\"><code>docs/reading-principles.md</code></a>.</p>\n<ul><li>Trigger: the reply gap shrinks against the person's own median, their share of started threads rises above their usual share, and the move holds across the period in more than one count (never a fixed number of days).</li><li>Confidence: graded. High when pace, initiation and reach all turn together; softer to \"maybe warming\" when only one count moves.</li><li>Needs: Meet (captured-call content) so contact and new names voiced on a call still count toward reach; when a thread moves off the connected accounts the reach count thins and confidence falls (a data gap, named not faked).</li></ul>"
    }
   },
   "sectionOrder": [
    "What it means",
 ...
 "cards": [
  {
   ...
   "sections": {
    "What I noticed": {
     "html": "<ul><li>Why now: Stephen's email on Wednesday ended \"we can't find that in this year's budget\". Left alone, that sentence becomes the decision.</li><li>Felton is at coalition: Shabana Akhtar and the service lead are for it. Stephen is finance, and his objection is the only one left.</li><li>Every message from Stephen mentions the annual figure. None questions what BOPS does. That is a price barrier, not a fit problem.</li><li>The partial rollout exists for exactly this: one service, live cases, about a third of the price, and a path to the rest next financial year.</li></ul>"
    },
    "What I read": {
    ...
    "Then": {
     "html": "<ul><li>After you send it I'll watch for whether Stephen replies himself or the service lead does. If it's the service lead, the price has stopped being the blocker.</li></ul>"
    },
    "The draft": {
     "html": "<p>Hi Stephen,</p>\n<p>You said the annual figure isn't in this year's budget, and I'd rather work with that than argue with it.</p>\n<p>Most councils don't start with the whole thing. The usual first step is one service on live cases only, which comes in at roughly a third of the figure you've seen. It is a smaller commitment, not a discount: the rest is there when the next budget round allows.</p>\n<p>The one-page breakdown is attached.</p>\n<p>Fede</p>"
    }
   },
   "sectionOrder": [
    "What I noticed",
  ...
  },
  {
   ...
   "sections": {
    ...
    "Then": {
     "html": "<ul><li>Once sent, I'll watch for Rachel's reply as much as Gary's. Her first message back tells us what kind of IT lead she is.</li></ul>"
    },
    "The draft": {
     "html": "<p>Hi Gary,</p>\n<p>Short answer: yes. BOPS imports existing case records, and most councils do it in stages rather than in one go. East Marston moved their open cases first and the archive later, over about six weeks. The case study is attached and has the detail.</p>\n<p>Rachel, good to meet you on the thread. The import notes are attached too, in case the how matters more than the story.</p>\n<p>If it helps, a partial rollout is the usual first step: one service, live records only, then widen. Happy to walk through what that looked like for East Marston.</p>\n<p>Fede</p>"
    }
   },
   "sectionOrder": [
    "What I noticed",
  ...
  },
  {
   ...
   "intro": "<p>She has asked \"will officers actually use it\" twice this week. The best answer is not from you. It is from someone in her job who has done it.</p>",
   "sections": {
    "What I noticed": {
  ...
  },
  {
   ...
   "title": "You were in the budget meeting at Bramley. Did it go badly for BOPS?",
   "intro": "<p>Karen has gone quiet since it, and I can't see what was said. Your answer changes what I do with her card.</p>",
   "sections": {
    "What I already know": {
     "html": "<ul><li>The meeting was on 22 August and you attended. Karen has replied once since, in two lines.</li><li>Nothing in your mail or Slack mentions how it went.</li></ul>"
    },
    "Where I ran out": {
    ...
    "What I'll do with your answer": {
     "html": "<ul><li>Badly: I hold Karen's note a month, look for a second door, and mark Bramley as at risk.</li><li>Fine: I send the note as drafted this week.</li><li>Don't know: I send the soft version and read her reply.</li></ul>"
    },
  ...
  },
  {
   ...
   "title": "You rewrite my openings every time. Should I drop the pleasantries?",
  ...
  },
  {
   ...
   "title": "You met Rachel Gill on Tuesday. How did it go?",
   ...
   "sections": {
    ...
    "What I'll do with your answer": {
     "html": "<ul><li>Warm: shorter, more direct drafts; the adoption question stops being a doubt.</li><li>Cautious: as now. Evidence, and peers rather than pitches.</li><li>Against: I route through Gary and keep her off the spot.</li><li>Didn't come up: nothing changes.</li></ul>"
    },
    "Where I'll remember it": {
  ...
  },
  {
   ...
   "sections": {
    ...
    "What I'll do with your answer": {
     "html": "<ul><li>For each you pick: I read their site and news, find the likely lead, and come back with a name and a first-note draft within a week.</li><li>The other nine wait. I won't touch them until these three have names.</li></ul>"
    },
    "Where I'll remember it": {
  ...
  },
  {
   ...
   "sections": {
    ...
    "What I can't": {
     "html": "<ul><li>Who you know. Who Simon knows. Who has moved where. Three cards this week say \"LinkedIn is not connected\" and stop there.</li></ul>"
    },
  ...
  },
  {
   ...
   "intro": "<p>Stephen Walsh's price question is really a priorities question, and priorities are set above him. Shabana has mentioned her director twice.</p>",
   "sections": {
    "What I noticed": {
     "html": "<ul><li>Why now: Stephen's \"not in this year's budget\" is a priorities call, and those are made above him. Widen the room before the price goes round again.</li><li>Shabana wrote \"David will want to see this\" in July and \"I'll need David onside\" in August. David Marshall, Director of Communities, is on no thread.</li><li>Two of three Felton voices are for it; the third is finance. Adding the director changes the shape of the decision.</li><li>This is a hunch more than a read: Shabana's two mentions are the whole evidence.</li></ul>"
    },
    "People": {
    ...
    "Then": {
     "html": "<ul><li>If Shabana agrees I'll draft the note for her to forward to David, in her voice, not yours. If she hesitates I'll drop it and let the price offer land first.</li></ul>"
    },
    "The draft": {
     "html": "<p>Hi Shabana,</p>\n<p>You've mentioned David a couple of times as the person who'd need to be onside. Would it help to bring him in now, before the price conversation with Stephen goes round again?</p>\n<p>I'm happy to send him a short note through you, or to do a twenty-minute session with the three of you. Whichever you think fits how Felton works.</p>\n<p>Fede</p>"
    }
   },
   "sectionOrder": [
    "What I noticed",
  ...
  },
  {
   ...
   "sections": {
    ...
    "The draft": {
     "html": "<p>Hi Sanjay,</p>\n<p>Julie Barker's auto-reply pointed me to you. We had been talking since April about BOPS, a case-work tool for officers, and she had a demo pencilled for the autumn.</p>\n<p>I don't want to assume any of that carries over. If it's useful, I can send a two-page summary of where we'd got to, or simply start again with whatever matters to you.</p>\n<p>Fede</p>"
    }
   },
   "sectionOrder": [
    "What I noticed",
  ...
  },
  {
   ...
   "title": "Tarlton is replacing its case system",
   "intro": "<p>Their tender went out on Wednesday and reads like your pitch. Darren Cole is the named contact. You have never spoken to anyone there.</p>",
   "sections": {
    "What I noticed": {
  ...
  },
  {
   ...
   "intro": "<p>Four notes from you since May, nothing back. A fifth would cost more than it gains. I'll stop suggesting them and watch for a warm path instead.</p>",
   "sections": {
    "What I noticed": {
     "html": "<ul><li>Why now: your fourth note went on 18 August and is four weeks unanswered. The pattern is clear enough to act on, and the act is to stop.</li><li>Four emails from you, May to August, to two people at Ashworth. No reply, no bounce, no auto-reply. Silence, not absence.</li><li>One-sided threads like this rarely turn with a fifth message. They turn with a different door: a person who knows them.</li><li>Your colleague Simon knows Nicola Shaw there. That is the Expand card next to this one.</li></ul>"
    },
    "Timeline": {
  ...
  },
  {
   ...
   "sections": {
    "What I noticed": {
     "html": "<ul><li>Why now: 22 days since her last real reply, three times her usual gap, and you have a gift ready: the figures she asked for.</li><li>Before the budget meeting she replied in about a day and started 9 of your 12 threads.</li><li>Since it: one two-line reply. No out-of-office, no leave, no meeting booked. She is active on other threads, so she is around.</li><li>Bramley is one of your thirty and she is the only door in. Your goal says move patiently, so give, don't nudge.</li></ul>"
    },
    "A number against its usual": {
     "html": "<ul><li>Now · 22 days · since her last real reply</li><li>Usually · about a day · her replies before the budget meeting</li></ul>"
    },
    "Timeline": {
    ...
    "The draft": {
     "html": "<p>Hi Karen,</p>\n<p>You asked back in July for the officer-time numbers from East Marston and I never sent them. Here they are, one page. The short version: case admin down by about a third, which is the figure their finance team cared about most.</p>\n<p>I imagine the budget round has been eating your weeks. No need to reply. If it is useful to talk through how East Marston phased it in, I am around whenever suits.</p>\n<p>Fede</p>"
    }
   },
   "sectionOrder": [
    "What I noticed",
  ...
  },
  {
   ...
   "sections": {
    ...
    "The draft": {
     "html": "<p>Hi Lisa,</p>\n<p>I read the strategy over the weekend. The commitment on page 12 to replace the legacy case systems by 2028 is the bravest line I have seen a council put in writing this year. Congratulations. It reads like a document someone actually intends to deliver.</p>\n<p>No agenda here. Just wanted you to know it landed.</p>\n<p>Fede</p>"
    }
   },
   "sectionOrder": [
    "What I noticed",
  ...
  },
  {
   ...
   "intro": "<p>You speak roughly every six weeks and it has been seven. He asked in August how East Marston phased the rollout, and now there is a note on it.</p>",
   "sections": {
    "What I noticed": {
     "html": "<ul><li>Why now: seven weeks since you last spoke, one past his rhythm, and the phasing note he asked about was finished on Friday.</li><li>Since March you and Neil have talked every five to seven weeks, usually a call he books after a short email from you.</li><li>He is warm but slow, and he doesn't chase. The pattern is that you open and he answers within two days.</li><li>Holbrook's procurement window closes 16 October. This is the gentle note; the follow-up card is the dated one.</li></ul>"
    },
    "What I read": {
  ...
  },
  {
   ...
   "intro": "<p>You promised her the data-residency note on 31 August. It is two weeks later and it hasn't gone.</p>",
   "sections": {
    "What I noticed": {
     "html": "<ul><li>Why now: it is two weeks since you wrote \"I'll send the data-residency note this week\", and you haven't. Every day makes the next email harder.</li><li>Rachel is Head of IT at Westmoor. Data residency is the first thing IT leads ask, and she asked it in her second message.</li><li>The note exists and is current. Nothing is stopping this except that it slipped.</li><li>Westmoor is your most advanced deal. An unkept promise to the newest person in it costs more than anywhere else.</li></ul>"
    },
    "Open items": {
     "html": "<ul><li>The data-residency note · You owe Rachel, promised 31 August · 14 days</li></ul>"
    },
    "Documents": {
    ...
    "The draft": {
     "html": "<p>Hi Rachel,</p>\n<p>I said I would send the data-residency note and then didn't. Sorry for the delay. It is attached: where the data lives, who can see it, and how it leaves if you ever want it to.</p>\n<p>If anything in it is unclear, or you want it in the form your information governance team prefers, say so and I'll adjust.</p>\n<p>Fede</p>"
    }
   },
   "sectionOrder": [
    "What I noticed",
   ...
   "widgetRows": {
    "open-items": [
     [
      ...
      "You owe Rachel, promised 31 August",
      "14 days"
     ]
    ],
    "documents": [
 ...
 "widgets": [
  ...
  },
  {
   ...
   "sections": {
    ...
    "Row form": {
     "html": "<p><code>&lt;when&gt; · &lt;what happened&gt;</code></p>\n<p>The first part is a date as the card would say it (\"22 August\", \"March\") or \"Today\". The second is one short sentence. The row marked \"Today\" is the filled dot.</p>"
    }
   },
   "sectionOrder": [
    "What it shows",
 ...
 "docs": [
  {
   ...
   "sections": {
    ...
    "The concepts underneath": {
     "html": "<ul><li><strong>Routine</strong> (steps 2–3): a standing job with one owner; it may pull from a <strong>supplier</strong> (People pulls the fit-shape from Offerings).</li><li><strong>Play</strong> (steps 4–6): the cross-module choreography Brain conducts, with the <strong>decision in the middle</strong> (step 5). Only Brain can own it.</li><li><strong>Provide</strong> (step 4): each module's piece, handed back when Brain asks.</li><li><strong>Brief → render</strong> (steps 6–7): Brain decides; Profile writes; Organisation gates.</li></ul>"
    },
    "Visualised — \"Trace the flow\"": {
     "html": "<p>This sequence is now playable on the canvas. A <strong>step timeline / scrubber</strong> (\"Trace the flow\") steps through all eight stages turn-by-turn: each lights the nodes and the directional wires it touches (reusing the route lines already drawn, marching in the flow direction) and shows that step's line.</p>\n<p>The trace <strong>follows the artifact</strong>: a single parcel rides the route and <strong>changes form only where a mind reasons</strong> — a raw <strong>fact</strong> that People's reading flips to a <strong>signal</strong>, then a <strong>brief</strong>, a <strong>draft</strong>, an <strong>action</strong>. The parcel is <strong>grey while it's a raw fetched record and turns blue the moment a mind reads it</strong>; that one grey→blue flip <em>is</em> the deterministic→non-deterministic boundary, shown not told. On the dumb legs it's just carried.</p>\n<p>The choreography — nodes, legs and the parcel's form per step — lives in <code>blueprint/lib/flow-trace.ts</code>, with the <em>words</em> mirrored from this doc; <strong>keep the two in sync.</strong> The framing follows the canvas's GPS metaphor (a route traced across the map) and the \"play = a path that lights up across nodes\" note in <code>ui-foundation.md</code>.</p>"
    }
   },
   "sectionOrder": [
    "Phase A — strategy becomes standing work",
  ...
  {
   "title": "The library format: how the modules' thinking is stored from here on",
   "intro": "<p>This is the <strong>contract</strong> for the structured library that replaces the one-prose-<code>CLAUDE.md</code>-per-module monolith. It is what an agent reads before producing entries. The matching skeletons live in <a href=\"../templates/\"><code>_templates/</code></a>; one worked example of each lives in its real home (linked below).</p>",
   "sections": {
    "Why we changed": {
     "html": "<p>The agent-anatomy prose was right for <em>reasoning</em>, but the part of each module that is really a <strong>catalog</strong> (the named, enumerable things at each floor of the pipeline) was trapped in prose, so it could not grow into a library or be rendered. We split each module in two:</p>\n<ul><li><strong>The catalog floors</strong> become a <strong>library of structured entries</strong> (this doc).</li><li><strong>The operating prose</strong> (principles, system prompt, user input, output, memory, open questions) becomes a slim <strong><code>module.md</code></strong> overview.</li></ul>\n<p>The mapping from the old agent-anatomy is exact:</p>\n<table><thead><tr><th>Old agent-anatomy section</th><th>New home</th></tr></thead><tbody><tr><td>Raw data</td><td><code>01-integrations/channels/*.md</code> (raw-data floor)</td></tr><tr><td>Reasoning, <em>assembling</em></td><td><code>&lt;module&gt;/assemblies/*.md</code></td></tr><tr><td>Reasoning, <em>sensing</em></td><td><code>&lt;module&gt;/signals/*.md</code></td></tr><tr><td>Principles, System prompt, User input, Output, Memory, Open questions</td><td><code>&lt;module&gt;/module.md</code></td></tr></tbody></table>"
    },
    ...
    "The worked examples (the gold standard to imitate)": {
     "html": "<ul><li>Channel: <a href=\"../channels/gmail.md\"><code>01-integrations/channels/gmail.md</code></a></li><li>Assembly: <a href=\"../assemblies/person-history.md\"><code>02-relationships/assemblies/person-history.md</code></a></li><li>Signal: <a href=\"../signals/cooling-champion.md\"><code>02-relationships/signals/cooling-champion.md</code></a></li><li>Module overview: <a href=\"../modules/02-relationships.md\"><code>02-relationships/module.md</code></a></li></ul>\n<p><strong>02-relationships is the worked example</strong>, so copy its shape.</p>"
    },
    "Deliberately deferred": {
     "html": "<ul><li><strong>Measures as their own floor.</strong> The deterministic counts are a <code>measures:</code> field on a signal for now. Because they are reusable and checkable (<code>reply-gap-vs-own-rhythm</code> feeds several signals), they may graduate to their own doc-per-entry floor. Open question, not yet.</li><li><strong>The higher floors</strong> (insights, briefs, actions) come once raw data, assemblies and signals prove out.</li><li><strong>The UI that renders the library.</strong> A first cut now exists: <code>/library</code> in the blueprint viewer is a filterable gallery of every assembly and signal (search · module · type · mode), each card mirroring its <code>label</code> + <code>blurb</code> + facets, with the body and clickable lineage in an overlay. The told floor is not rendered there yet. Retiring the old per-module <code>CLAUDE.md</code>s is <strong>done</strong>.</li></ul>"
    }
   },
   "sectionOrder": [
    "Why we changed",
 ...
 "templates": [
  {
   ...
   "about": "<a person|a conversation|an organisation>",
   "deterministic": true,
   "inputs": [
 ...
 "records": [
  ...
  },
  {
   ...
   "fields": [
    ...
    {
     "field": "style (nurture a few ↔ work many)",
     "source": "you, seeded by archetype, refined by behaviour"
    },
    {
